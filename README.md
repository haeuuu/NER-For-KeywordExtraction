...
#### 1 ) [Naver/창원대 NLP Challenge의 train set](http://air.changwon.ac.kr/?page_id=10)
...
**개체명의 의미**는 `2. 유형(Type)별 단어 검색` 의 `유형` 분류에 적힌 것을 기준으로 `1 ) Naver/창원대 train set`에 제시된 12개의 tag 중 하나로 적용한다.

* ex ) `단체` 카테고리에 속하는 `간민회` , `강구회` 등의 단어는 정답 tag는 `ORG`로 약속한다.

**개체명**은 `유형` 분류에 들어가 스크래핑 한다.

* `ORG`가 정답 tag인 개체명 데이터셋을 생성하기 위하여 `유형별 단어 검색 > 단체`  에 들어가 단어를 고른다.

**문장**은 해당 개체명의 설명 페이지(아래 그림)에서, 개체명이 들어간 문장을 고른다.
...
NER 학습을 위해서 Naver와 창원대에서 개최한 NLP Challenge에서 제공하는 train set을 가져왔다.
