...
* 한국사 관련 용어에는 `강릉/매화/타령`과 같이 여러 개체명이 함께 붙어있는 경우가 많다. 

  기존 모델(Data1로만 학습한 모델)의 규칙은 해치지 않으면서도 한국사를 학습시키기 위해서는 먼저 기존 모델이 각 단어를 어떻게 tagging하는지를 확인할 필요가 있다.

* 기존 모델로 prediction한 결과 



이 경우에 **앞의 개체명을 따를 것인지 뒤의 개체명을 따를 것인지**는 개체명 종류에 따라 달리하였다.

1. 뒤를 기준으로 tagging한 경우 : 작품(AFW), 사건(EVT), 유물(AFW)
   * 작품이나 유물의 경우, 앞에 지명이 달리는 경우가 많음.
     ex ) `강릉매화타령`, `경주검총 `
   * 뒤의 타령, 검총을 따라 tagging하는 것이 맞다고 판단.
2. 앞을 기준으로 tagging한 경우 : 유적(LOC), 제도(ORG), 지명(LOC)
